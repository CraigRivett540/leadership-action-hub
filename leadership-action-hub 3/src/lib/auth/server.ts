...
/** Vercel auto-injects these hostnames (no protocol). */
function vercelEnvOrigins(): string[] {
  const out: string[] = [];
  for (const key of [
    "VERCEL_URL",
    "VERCEL_BRANCH_URL",
  ...
  // Production custom / project domains set by the team
  if (explicitBaseURL) {
    out.push(explicitBaseURL.replace(/\/$/, ""));
  }
  return [...new Set(out)];
}

// Prefer fixed public URL when set; otherwise resolve per-request (preview / Vercel).
const vercelHosts = vercelEnvOrigins().map((o) => o.replace(/^https?:\/\//, ""));
...
// Always include Vercel URLs + wildcards so preview/production deployments work
// even when BETTER_AUTH_URL is missing or only lists one domain.
const trustedOrigins: string[] = [
  ...vercelEnvOrigins(),
  ...LOCAL_DEV_ORIGINS,
  // Host / origin wildcards (Better Auth matches Origin against these)
  ...previewAllowedHosts,
