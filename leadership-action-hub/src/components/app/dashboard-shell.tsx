...
export function DashboardShell() {
  ...
  const filteredActions = useMemo(() => {
    ...
    if (!isAdmin && profile) {
      list = list.filter((a) => {
        if (a.assigneeId === profile.id || a.createdBy === profile.id) return true;
        const g = groups.find((x) => x.id === a.assigneeId);
        return Boolean(g?.memberIds.includes(profile.id));
      });
    }
    if (statusFilter === "open") list = list.filter((a) => a.status === "open");
    if (statusFilter === "closed") list = list.filter((a) => a.status === "closed");
    if (statusFilter === "overdue") list = list.filter(isOverdue);
    return list;
  }, [actions, isAdmin, profile, groups, statusFilter]);
  ...
  const nav: NavItem[] = isAdmin
    ...
    : [
        { kind: "view", id: "overview", label: "Overview", icon: LayoutDashboard },
        { kind: "view", id: "all-actions", label: "My actions", icon: CheckSquare },
        { kind: "view", id: "my-todo-work", label: "My to do · Work", icon: ListTodo },
        ...staff
          .filter((s) => s.active && s.role === "admin")
          .sort((a, b) => a.name.localeCompare(b.name))
          .map((s) => ({
            kind: "staff" as const,
            staffId: s.id,
  ...
  function openNewAction(opts?: {
    ...
    if (
      ...
      setAAssignee(opts?.prefillAssignee ?? teamStaff[0]?.id ?? ADMIN_ID);
      setAOrg("hh");
    } else {
      setAType("request");
      setAAssignee(ADMIN_ID);
      setAOrg(profile.organisation === "both" ? "hh" : profile.organisation);
    }
    setActionOpen(true);
  }
        ...
        <main className="min-w-0 flex-1 px-3 py-5 sm:px-6 sm:py-6">
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h2 className="text-xl font-bold text-hh-navy">
                  {isAdmin ? "All actions" : "My actions"}
                </h2>
                <div className="flex flex-wrap gap-2">
          ...
          {view === "my-todo-work" && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {workTodoItemsForUser(actions, profile.id).map((a) => (
                  <ActionCard key={a.id} action={a} />
                ))}
                {workTodoItemsForUser(actions, profile.id).length === 0 && (
                  <Empty>No work to-do items yet.</Empty>
                )}
              </div>
            </div>
          )}
          ...
          {view === "family" && isAdmin && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {familyItemsForUser(actions, profile.id).map((a) => (
                  <ActionCard key={a.id} action={a} />
                ))}
                {familyItemsForUser(actions, profile.id).length === 0 && (
                  <Empty>No family items yet.</Empty>
                )}
              </div>
            </div>
          )}
          ...
          {view === "group-detail" && selectedGroup && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {actions
                  .filter((a) => isTeamAction(a) && a.assigneeId === selectedGroup.id)
                  .map((a) => (
                    <ActionCard key={a.id} action={a} />
                  ))}
              </div>
            ...
            <DialogDescription>
              {isFamilyType(aType)
                ...
                : isWorkTodoType(aType)
                  ...
                  : "Team tasks and requests are shared. Use My to do · Work or Family for private lists."}
            </DialogDescription>
          </DialogHeader>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (isPrivateType(next)) {
                    ...
                    setAAssignee(isAdmin ? (teamStaff[0]?.id ?? ADMIN_ID) : ADMIN_ID);
                  }
                }}
              >
                {isAdmin ? (
                  <>
                    <optgroup label="Team (shared)">
                      <option value="task">Task</option>
                      <option value="request">Request</option>
                    </optgroup>
                    <optgroup label="My to do · Work">
                  ...
                  <>
                    <optgroup label="Team">
                      <option value="request">Request to leadership</option>
                    </optgroup>
                    <optgroup label="My to do · Work">
                      <option value="personal_task">Work task</option>
                      <option value="personal_request">Work request</option>
