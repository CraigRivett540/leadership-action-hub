...
async function loadBundle() {
  ...
  return {
    ...
    actions: actionRows.map((a) => ({
      ...
      files: (() => {
        try {
          return JSON.parse(a.files_json) as { name: string; size: number; type?: string; dataUrl?: string }[];
        } catch {
          return [];
        }
      })(),
...
export const saveActionRecord = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const existing = await sql<{ created_by: string }>`
      select created_by from actions where id = ${data.id} limit 1
    `;
    if (existing[0] && existing[0].created_by !== staffId) {
      await sql`
        update actions
        set files_json = ${JSON.stringify(data.files ?? [])}
        where id = ${data.id}
      `;
      return { ok: true };
    }
...
export const toggleActionRecord = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data: id }) => {
    ...
    const rows = await sql<{
      ...
      select status, created_by, assignee_id from actions where id = ${id} limit 1
    `;
    if (!rows[0]) throw new Error("Action not found");
    const row = rows[0];
    const memberRows = await sql<{ staff_id: string }>`
      select staff_id from group_members where group_id = ${row.assignee_id}
    `;
    const involved =
      row.created_by === staffId ||
      row.assignee_id === staffId ||
      memberRows.some((m) => m.staff_id === staffId);
    if (!involved) throw new Error("Only people involved can complete or reopen this item");
    const next = row.status === "open" ? "closed" : "open";
    await sql`update actions set status = ${next} where id = ${id}`;
    return { ok: true, status: next };
  });

export const removeActionRecord = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator((id: string) => id)
  .handler(async ({ context, data: id }) => {
    const staffId = await staffIdForAuthUser(context.userId);
    ...
    const rows = await sql<{ created_by: string }>`
      select created_by from actions where id = ${id} limit 1
    `;
    if (!rows[0]) throw new Error("Action not found");
    if (rows[0].created_by !== staffId) {
      throw new Error("Only the person who posted this item can delete it");
    }
    await sql`delete from action_notes where action_id = ${id}`;
